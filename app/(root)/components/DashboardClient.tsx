        ...
        <Card className="p-5 shadow-sm border border-border bg-card flex flex-col">
          ...
          <div className="flex flex-col gap-4 flex-1">
            {(() => {
              ...
              const currentMonth = data.monthlyPerformance.find(
                (item) => item.month === currentMonthNumber,
              );

              const previousMonth = data.monthlyPerformance.find(
              ...
              const insights = [
                ...
                },
                {
                  ...
                  previous: previousMonth?.profit ?? 0,
                  change: calculateChange(
                    currentMonth.profit,
              ...
              return insights.map((item, index) => {
                const increase = item.change >= 0;

                // Determines whether the result is good or bad.
                // Sales & Profit: increase = good.
                // Expenses: decrease = good.
                const isGood = item.isExpense ? !increase : increase;

                return (
                  <div
                    key={index}
                    className="rounded-xl border border-border p-4 hover:bg-muted/30 transition-all"
                  >
              ...
              <TableBody>
                {(() => {
                  ...
                  return rows.map((item, index) => {
                    const growth = item.changeFromPrev;
                    const positive = growth >= 0;
                    const relativeOffset = rows.length - 1 - index;

                    const label =
                      relativeOffset === 0
                        ? `${item.monthName} (This Month)`
                        : relativeOffset === 1
                          ? `${item.monthName} (Last Month)`
                          : relativeOffset === 2
                            ? `${item.monthName} (2 Months Ago)`
                            : `${item.monthName} (3 Months Ago)`;
